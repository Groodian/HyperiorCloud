    ...
    public void handle(LogEntry logEntry) {
        if (!maxFileSizeReached) {
            if (file.length() < MAX_FILE_SIZE) {
                ...
                LogEntry errorLogEntry = new LogEntry(LogLevel.FATAL, System.currentTimeMillis(),
                        "Max file size of " + MAX_FILE_SIZE + " bytes for file " + file.toString() + " reached!", null,
                        Thread.currentThread(), this.getClass());
