...
import de.groodian.hyperiorcloud.event.events.PreServiceConnectedEvent;
...
import de.groodian.hyperiorcloud.service.connections.ProxyServiceConnection;
import de.groodian.hyperiorcloud.service.services.PaperService;
import de.groodian.hyperiorcloud.service.services.VelocityService;
...
public class ServiceHandler implements Listener {
    ...
    public void startService(ServiceType serviceType, String group) {
        ...
        switch (serviceType) {
            case PAPER:
                services.add(new PaperService(this, group, groupNumber, port));
                break;
            case VELOCITY:
                services.add(new VelocityService(this, group, groupNumber, 25565));
                break;
            default:
                Master.getInstance().getLogger().error("[ServiceHandler] Unknown service type: " + serviceType);
                break;
    ...
    public void handlePreServiceConnected(PreServiceConnectedEvent e) {
        for (Service service : getServices()) {
            if (service.getId().equalsIgnoreCase(e.getGroup() + "-" + e.getGroupNumber())) {
                if (e.getGroup().equalsIgnoreCase("PROXY")) {
                    service.setConnection(new ProxyServiceConnection(e.getConnection(), service));
